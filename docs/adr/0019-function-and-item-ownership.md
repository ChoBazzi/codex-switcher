...
- custom_tool_call, 로컬 shell 전용 형식, 이미지/파일 결과는 아직 영속 경로에서 지원하지 않는다. ADR 0021에서 관찰한 내용 해시로 검증하는 assistant/reasoning 이력과 완전한 additional_tools 정의를 추가했다. 기존 live-test의 별도 additional_tools 지원은 유지한다.
...
- ADR 0020에서 CLI 시작 이벤트 기반 등록 및 영속 프록시 경유 합성 함수 왕복을 추가 검증했다. 일반 CLI 런처 전체 호환성을 의미하지 않는다.
