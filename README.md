...
**앱에서 Codex 파일 설정:** `Codex 설정`에서 앱 연결용 `CODEX_HOME`의 `config.toml`과 `AGENTS.md`를 탭으로 열어 직접 편집하고 파일별로 저장합니다. 없는 파일은 저장할 때 생성합니다. 외부에서 변경된 파일은 덮어쓰지 않으며, 초안을 복사해 둔 뒤 다시 불러오세요. 저장 후 현재 CLI 작업을 마치고 종료한 뒤 대화 재개 명령으로 같은 기록을 선택합니다. TOML 문법은 CLI 실행 시 확인하며 프록시 연결과 재시도 0 설정은 유지해야 합니다. [ADR 0056](docs/adr/0056-app-codex-launch-settings.md)
