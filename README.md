...
앱은 독립 `proxy-daemon`에 relay로 연결합니다. **앱 종료·재실행 후에도 기존 CLI와 프록시 상태를 유지**하도록 구현했으며, 앱의 `모델 프록시 다시 연결`은 기존 서비스에 연결합니다. Go/Swift 빌드와 메모리 제어 검사는 통과했고, 실제 로컬 서버 통합 검사는 실행 환경의 bind 제한으로 남아 있습니다. 기존 앱 소유 helper 종료 설명은 [ADR 0054](docs/adr/0054-independent-proxy-service.md)로 대체합니다.

새 helper 적용 시 기존 CLI 턴을 마치고 앱을 종료한 뒤 `./bin/switcher-helper proxy-stop`으로 **이미 실행 중인 독립 서비스**를 종료합니다. 처음 이 버전으로 전환할 때 기존 앱 소유 helper는 앱 종료로 끝나므로 `proxy-stop`은 필요 없습니다. 이후 새 앱을 실행하고 새 연결 명령으로 CLI를 시작합니다. 요청·도구 턴 중 서비스 종료는 거절되며 실패 요청을 재전송하지 않습니다. 앱이 꺼져 있어도 사용량 정기 조회는 계속됩니다. daemon 자체 종료/재부팅 후 기존 대화 자동 복원은 지원하지 않습니다.
