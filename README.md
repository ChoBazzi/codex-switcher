...
**앱·프록시 함께 시작/종료:** 앱의 `앱·프록시 종료`와 일반 종료 요청은 프록시 종료를 확인한 뒤 앱을 닫습니다. 요청·도구·계정 작업 중이면 안내하고 종료하지 않습니다. 다시 앱을 켜면 기존 주소와 대화를 복구하므로 같은 CLI에서 새 입력을 보낼 수 있습니다. 앱이 꺼진 동안에는 모델 요청을 보낼 수 없습니다. 아래 과거 앱 단독 종료 설명보다 [ADR 0059](docs/adr/0059-app-and-proxy-lifecycle.md)가 우선합니다.

**리뷰·서브에이전트·자동 승인:** 같은 앱 연결의 보조 대화를 별도 처리해 `probe_identity_invalid` 문제를 수정했습니다. 보조 작업은 시작 계정에 고정되며 작업 중 계정 전환은 잠깁니다. 실패하거나 프록시를 재시작한 보조 작업은 자동 재전송하지 않고 새 작업으로 시작합니다. 보조 native 압축과 다른 루트 대화(`/new`, `/fork`)의 동일 연결 공유는 지원하지 않습니다. 새 helper 적용이 필요합니다. [ADR 0058](docs/adr/0058-auxiliary-conversation-isolation.md)

**한도 오류 후 복구:** 연결된 실패 상태에서는 `사용 가능한 계정으로 복구 준비`를 누릅니다. 최신 잔여량 5% 초과 계정을 선택하되 요청은 전송하지 않습니다. CLI가 입력창으로 돌아왔는지 확인한 뒤 `현재 파일 상태부터 확인하고 아직 안 된 작업만 계속해줘`처럼 새 지시를 입력하세요. 앱 재실행은 실패 잠금을 없애지 않습니다. 연결이 끊겼으면 `모델 프록시 다시 연결`로 먼저 연결하고 복구를 준비합니다. [ADR 0057](docs/adr/0057-explicit-quota-failure-recovery.md)

**앱에서 Codex 파일 설정:** `Codex 설정`은 위젯 내부에서 열리며 `계정 목록으로`로 돌아갑니다. 앱 연결용 `CODEX_HOME`의 `config.toml`과 `AGENTS.md`를 탭으로 열어 직접 편집하고 파일별로 저장합니다. 없는 파일은 저장할 때 생성합니다. 외부에서 변경된 파일은 덮어쓰지 않으며, 초안을 복사해 둔 뒤 다시 불러오세요. 저장 후 현재 CLI 작업을 마치고 종료한 뒤 대화 재개 명령으로 같은 기록을 선택합니다. TOML 문법은 CLI 실행 시 확인하며 프록시 연결과 재시도 0 설정은 유지해야 합니다. [ADR 0056](docs/adr/0056-app-codex-launch-settings.md)

**프록시 종료 후 대화 복구:** 독립 프록시가 죽어도 연결 주소·CLI 홈·대화와 계정 상태를 비공개 로컬 checkpoint에 보존합니다. 앱의 `모델 프록시 다시 연결` 또는 앱 재실행 후 **기존 CLI에 새 지시를 입력**해 이어갑니다. 요청/도구 턴 중 종료됐다면 CLI가 입력창으로 돌아왔는지 확인하고 현재 계정의 `복구 준비 · 새 입력 대기`를 누른 뒤 새 지시를 입력하세요. 실패 요청과 도구는 자동 재전송/재실행하지 않습니다. 불완전 도구 이력은 새 대화가 필요할 수 있습니다. [ADR 0055](docs/adr/0055-proxy-crash-session-recovery.md)

이 기능은 새 helper와 앱을 적용한 이후 생성한 연결부터 지원합니다. 이미 종료된 구형 프록시의 상태는 소급 복구하지 못합니다. 앱 종료 → 기존 서비스의 `./bin/switcher-helper proxy-stop` → 새 앱 실행 → 새 연결 명령 순서로 최초 적용합니다. 이후 일반 `proxy-stop`은 상태를 보존하며, **새 대화를 의도적으로 시작할 때만** 앱 종료 후 `./bin/switcher-helper proxy-stop --new-session`으로 기존 연결을 종료하고 새 연결 명령을 받습니다. 과거 CLI 기록은 삭제하지 않습니다.

재부팅으로 CLI도 종료됐다면 앱을 켜고, 연결 명령의 동일한 `CODEX_HOME`을 사용해 `codex resume`으로 기록을 선택합니다. 실행 중이던 터미널/요청 자체를 자동 복원하지는 않습니다. 저장 오류·포트 충돌·확인 불가한 암호화 소유권은 안전하게 차단합니다.

**토큰 자동 갱신·독립 프록시 서비스:** 앱 프록시는 사용량 조회/모델 전송 전에 만료가 2분 이내인 토큰을 갱신합니다. 갱신 실패·응답 유실 후에는 자동 재교환하지 않고 `다시 로그인`을 안내합니다. 실제 OAuth 응답 호환성 검증은 남아 있습니다. [ADR 0053](docs/adr/0053-helper-token-refresh.md)

앱은 독립 `proxy-daemon`에 relay로 연결합니다. **앱 종료·재실행 후에도 기존 CLI와 프록시 상태를 유지**하도록 구현했으며, 앱의 `모델 프록시 다시 연결`은 기존 서비스에 연결합니다. Go/Swift 빌드, 합성 로컬 서버 통합 검사와 설치 CLI 재시작 검사를 통과했습니다. 기존 앱 소유 helper 종료 설명은 [ADR 0054](docs/adr/0054-independent-proxy-service.md)로 대체합니다.

새 helper 적용 시 기존 CLI 턴을 마치고 앱을 종료한 뒤 `./bin/switcher-helper proxy-stop`으로 **이미 실행 중인 독립 서비스**를 종료합니다. checkpoint 지원 버전으로 만든 연결은 새 앱을 실행한 뒤 기존 CLI에서 이어갑니다. 구형 버전에서 최초 전환할 때만 새 연결 명령으로 CLI를 시작합니다. 요청·도구 턴 중 서비스 종료는 거절되며 실패 요청을 재전송하지 않습니다. 앱이 꺼져 있어도 사용량 정기 조회는 계속됩니다. daemon 종료 후의 복구는 위 ADR 0055를 따릅니다.

검증 명령: `go test -race ./internal/accounts` 및 `go test -race ./cmd/switcher-helper -run 'TestService'`, 전체 검사는 `sh macos/Checks/verify.sh`입니다. 실제 사용자 검증에서 파일 수정·계정 전환 및 취소·복구·계정 관리 검사는 완료했으며, 잔여량 5% 이하 자동 전환은 실사용 중 확인 예정입니다. 새 수명 분리 기능은 합성 스트리밍 중 앱 종료/재연결과 별도로 실제 앱에서도 확인해야 합니다.
